...
**⚠ WHAT "NEW SURFACE" MEANS, because it was read too literally on 2026-08-09 and produced a `0.6.0`
that should have been a `0.5.1`.** It means **the mod does something it did not do before**. A cvar or
console command added *only so a repair can be configured* is part of that repair and stays PATCH —
`FPM.Diag.Clear` exists because the overlay printed forever, and `FPM.Diag.OverlayTop` exists because
it covered the player's own HUD. Neither is a capability. Ant: *"why the big bump for a small patch?
you should be better at bumping the proper way."* **The test to apply: could you describe the release
without mentioning the new command? If yes, it is plumbing, and it is a PATCH.**
...
## 2026-08-09 11:15 — VERSION — 0.5.1 → 0.6.0

- **What:** `VersionName` / `SemVersion` → `0.6.0`; pin generated by `stamp_version.py --stamp`.
- **Why MINOR, by the same test that made 0.5.1 a PATCH:** *could you describe this release without
  mentioning the new commands?* **No — the commands ARE the release.** The mod can now read a cvar's
  full priority-layer stack and hold a cvar on demand; it could do neither before. That is new
  capability, not plumbing for a repair.
